...
char     LogName[128]="coewo2_6.log";
char     MappName[128]="inicjacja.gif";
...
wb_dynarray<agent>	  ziemia;  //Wlaœciwa warstwa agentow zasiedlajacych
...
////pom1->settitleback(64/*default_transparent*/);//Ale mozna uwidocznic
//Menager.insert(pom1);
//}
...
//TEST BACKGROUNDU - PRZYCISK NA WIERZCHU
...
pom=new carpet_graph(Menager.getwidth()-100,0,Menager.getwidth()-1,20,//domyslne wspolrzedne						
...
for(long i=0;i<ile;i++) 
	...
	unsigned a=RANDOM(8),licznik=0;	
	int x1,y1;
	do{
	if((licznik++)==8) goto JESZCZE_RAZ;
	assert(a<8);
	x1=(x+kierunki[a].x);
	y1=(y+kierunki[a].y);
	Torus(x1,y1);
	}while(zdatnosc.get(x1,y1)==0);
...
if(!Lufciki.start("CO-EVOLUTION version 2.6",argc,argv))
		{
...
for(int i=1;i<argc;i++)
    ...
    fprintf(stderr,"BOK=NN BUM=NN LOG=N PLIK=name.log MAPP=init.gif\n");
	return 0;
	}
    }
return 1;
