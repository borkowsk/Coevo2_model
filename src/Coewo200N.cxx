...
public:
    ...
    my_area_manager(size_t size, //Konstruktor dający zarządcę o określonym rozmiarze listy
...
for(int ka=0;ka<BITS_PER_GENOM;ka++)
    log.insert(histspecjA->Categories(ka));
for(int kh=0;kh<BITS_PER_GENOM;kh++)
    log.insert(histspecjH->Categories(kh));
...
swiat::swiat(size_t szerokosc, char* logname, char* mappname, my_area_manager& AreaMenager):
...
my_area_manager Lufciki(255, SWIDTH, SHEIGHT, 28); //255 możliwych lufcików
...
// Zwraca 1 jeśli   ok
int my_area_manager::start(const char* wintitle, int argc, const char** argv, int double_buffering)
{
...
int my_area_manager::_pre_process_input(int input_char)
{
