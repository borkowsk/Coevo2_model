/* Program symulujÄcy KOEWOLUCJÄ (Wersja z roku 2013 - prawdopodobnie reaktywowana na nową wersję BDS)*/
/* KaÅŒdy osobnik ma swÃ³j bitowy wzorzec odÅŒywiania i bitowy wzorzec */
/* strategii osÅony. JeÅli ATAKOWANY.OSLONA AND ATAKUJACY.Trofia>0 to  */
/* znaczy ze atak zakoÅczyÅ sie powodzeniem.			    */
/* Osobniki rozmnaÅŒajÄ sie kosztem wolnego miejsca i zgromadzonej energii */
/* Ruchy wÅasne, ofiara ataku, jak i moment rozmnaÅŒania wybierane sÄ */
/* losowo, aby nie zaciemniaÄ modelu dodatkowymi parametrami. 	     */
//W razie czego:  __sbh_heap_check  albo podobne jeÅli jestâŠ
...
unsigned lang_selector=1;     //Jaki jÄzyk
//WERSJA 3.61
//1. MoÅŒliwoÅÄ puszczania z tym samym ziarnem losowym

//WERSJA 3.6
//1. Angielskie nazewnictwo okien i oraz tÅumaczenie innych elementÃ³w
//2. Zmienne w czasie nasÅonecznienie
//3. Angielskie nazwy parametrÃ³w i komunikaty

//Wersja 3.5
//UporzÄdkowanie licznikÃ³w krokÃ³w - przejÅcie na kroki Monte-Carlo tam gdzie siÄ da...
//ZostaÅy licznik krokÃ³w oraz osobno liczone 'wersje' danych.
//Wizualizacja 'on run' serii czasowych daje zawsze rÃ³wne odstÄpy punktÃ³w, niezaleÅŒnie
//od aktualnej multiplikacji MC. Ale w pliku log jest seria z wÅaÅciwymi wartoÅciami MC

//Wersja 3 - nowe moÅŒliwoÅci
//1. RÃ³ÅŒne dane do kolorowania drzewa filogenetycznego wybierane z menu
//2. Rozbudowa listy parametrÃ³w 'wyprowadzonych' na zewnÄtrz programu i usuniÄcie blokady na 'Åwiaty' wiÄksze od ekranu
//3. Dodanie miernika czasu procesora w gÅÃ³wnej pÄtli i wyprowadzenie wraz z krokiem na konsole
//4. 25.04.08 UsuniÄcie bÅÄdu w bibliotecznym menagerze lufcikÃ³w wywalajÄcego program przy robieniu TILE
//5. 11.09.08 Wypisywanie do pliku kompletnej filogenezy
//6. Wersja 3.1 - poprawki techniczne i kompilowalnoÅÄ pod MSVC 2008
//7. Uelastycznienie wizualizacji sieci ekologicznej i jej zapisywania
//8. UmoÅŒliwienie ciÄgÅych zrzutÃ³w zawartoÅci okna graficznego i sieci ekologicznej w dwu formatach
//9. Regulacja czÄstoÅÄ zrzutÃ³w - NIEDOKOÅCZONA - POWODUJE BÅÄDNÄ NUMERACJE PLIKÃW I KROKÃW

//Istotne nowosci w wersji 2.5
//1. OdwrotnoÅÄ prawdopodobieÅstwa mutacji wyprowadzona jako parametr RTG
//Mutacje w okolicy 1/16000 dla obszary 400x200 doprowadzajÄ do wyparcia heterotrofÃ³w 70000:100
//i to przy roÅŒnych wartoÅciach globalnoÅci oddziaÅywaÅ.
//Przy poziomie 1/1600 jest zachowana rÃ³wnowaga podobna do tej z typowego poziomu 1/160
//2. Wprowadzenie parametru wewnÄtrznego `ZAMIANY' umoÅŒliwiajÄcego elastyczne przepÅywanie obok siebie obojÄtnych osobnikÃ³w
//Ma ono zapobiegaÄ formowaniu sie frontÃ³w - czyli blokowaniu rozprzestrzeniania sie skutecznych taksonÃ³w przez 'gÄstoÅÄ zaludnienia'
//3. Wprowadzenie maksymalnego dystansu skoku pozwala zbadaÄ zaleÅŒnoÅÄ od 'globalnoÅci ekologii'
//4. Dodano tez wyÅwietlanie wieku taksonÃ³w
//5. Dodano moduÅ 'kladystyka' oparty o dane z âklonalinfoâ i sÅuÅŒÄcy preparowaniu prymitywnego (na razie) drzewa filogenetycznego
//   kolorowanego specjalizacjÄ
//6. Nieco uelastyczniono sposÃ³b ustalania co jest klonem a co taksonem - treshold jest teraz zmienna
//7. Dodano moduÅ ekologiczny rejestrujÄcy i analizujÄcy siec troficzna
//8. UzupeÅniono interface o moÅŒliwoÅci wÅasnych opcji uÅŒytkownika - np. wyÅÄczanie obliczania sieci ekologicznej i kladystyki
//9. Zmieniono na szary tÅo niektÃ³rych grafÃ³w
//15.02.2006
//  Rekompilacja z poprawionÄ bibliotekÄ wspierajÄca
//  ...
...
#define HIDE_WB_PTR_IO	(0) //Musi byÄ IO dla wb_dynarray
...
class swiat; //Zapowiedz gÅÃ³wnej klasy symulacji

//Menager lufcikÃ³w ze uzupeÅniona obsÅuga i/o (z menu)
class my_area_menager:public main_area_menager
{
	bool trace_fillog_tree;
	bool trace_trophi_netw;
	swiat*   MojSwiat;
	virtual int _pre_process_input(int input_char); //Przed obsÅugÄ domyÅlnÄ. Zwraca 1 jeÅli obsÅuÅŒyÅ.
public:

	my_area_menager(size_t size, //Konstruktor dajÄcy zarzÄdcÄ o okreÅlonym rozmiarze listy
				...
				MojSwiat(NULL)
	{}
	int start(const char* wintitle,int argc=0,const char** argv=NULL,int double_buffering=-1);//Zwraca 1 jeÅli ok
...
//BARDZO WAÂ¯NE WSKAÂ¯NIKI DO MASEK CZYLI GENÃW BÃDÂ¥CYCH SKÂ£ADOWYMI STRUKTURY
//base agent::* geba_ptr=&agent::w.w.geba;      //TAK POWINNO BYC - ALE NIE KOMPILUJE
//base agent::* oslona_ptr=&agent::w.w.oslona;

#define RECZNIE_WPISZ_PTR //RECZNE WPISYWANIE - ZALEZNE OD 'ENDIAN' I WIDZIMISIE TWORCÃW KOMPILATORA

#ifdef  RECZNIE_WPISZ_PTR
base agent::* geba_ptr=NULL; //Tu w zasadzie tylko deklaracja...
base agent::* oslona_ptr=NULL;//Przypisanie jest dalej, teÂ¿ zabezpieczone ifdef RECZNIE_WPISZ_PTR
#else
base agent::* geba_ptr=&(agent::w.w.geba); //TAK NIE DZIAÂ£A
base agent::* oslona_ptr=&(agent::w.w.oslona);
#endif
...
bool		UZYWAJ_PELNE_PROM=true;				//Czy uÂ¿ywamy peÂ³nego promieniowania, czy obniÂ¿onego?     NIE UÂ¯YWANE???

int    WSP_KATASTROF=0;//10-100		// Wykladnik rozkladu katastrof - 0 -wylaczone
int    DZIELNIK_1_OKRESU_MILANKOVICA=50;   //Przez ile dzieliÃŠ kroki M C Â¿eby uzuskac cos do zmian oÅwietlenia
double POWER_COS_NASLONECZNIENIA=0;        //"PotÃªga cyklu sÂ³onecznego": 0 - wyÂ³Â¹czone  1 - zwykÂ³y cosinus   2 - tropikalny 3 - pory roku
...
//ZewnÃªtrzne zarzÂ¹dzanie symulacjÂ¹ i zbieraniem danych
...
int          MonteCarloMultiplic=10; //Ewentualna modyfikacja multiplikacji dlugosci kroku MonteCarlo
									//... ale dlaczego byÅo double ??????
unsigned dlugosc_logow=100000; //DlugoÅÃŠ wewnetrznych logÃ³w podrÃªcznych - do wizualizacji
...
//-----------------------------------------------------------------------------
{
	...
	{   //Bardzo prymitywne, bez kontroli bÂ³ÃªdÃ³w!!!
...
double 			globalneSwiatlo; //Bazowa globalna iloÅÃŠ ÅwiatÂ³a -  staÂ³a
double 			AktualneSwiatlo; //Aktualna iloÅÃŠ swiatÂ³a - moÂ¿e siÃª zmieniaÃŠ w czasie

// Statystyki i liczniki
/////////////////////////////////
unsigned long	licznik_krokow_w;// licznik wywoÂ³an procedury kroku symulacji
...
rectangle_layer_of_agents<agent>    ziemia; //WlaÅciwa warstwa agentow zasiedlajacych
...
net_graph*			TrophicNet;		//SieÃŠ zaleznoÅci pokarmowych
...
//DostÃªp do gÂ³ownych danych
agent& Ziemia(size_t Kolumna,size_t Wiersz) {return ziemia.get(Kolumna,Wiersz);} //DostÃªp do ziemii
float  Swiatlo(size_t Kolumna,size_t Wiersz) {return globalneSwiatlo;} //DostÃªp do swiatla - tu globalnego
...
unsigned long   daj_kroki_monte_carlo(){return monte_carlo_licz;}   //suma realnie wykonanych krokÃ³w MC
void print_params(ostream& out);
//Do dodatkowych logÃ³w
...
//ZRODLA SUROWYCH WLASCIWOSCI AGENTÃW
...
if(!pom) goto DO_OBSLUGI_BLEDOW;
//pom->setdatacolors(0,255);//Pierwsze 25 kolorow bedzie slabo widoczne
pom->settitle(lang("PODATNOÅÃ AGENTÃW NA ATAK (maski obrony)","SENSITIVITY TO THE ATTACK (mask of defence)"));
pom->settitlecolo(255,default_transparent);
pom->setbackground(default_half_gray);
MyAreaMenager.insert(pom);
...
pom->setbackground(default_half_gray);
pom->settitle(lang("ZASOBY ENERGII AGENTÃW","ENERGY RESOURCES OF AGENTS"));
pom->settitlecolo(255,default_transparent);
MyAreaMenager.insert(pom);
...
pom->setbackground(default_half_gray);
pom->settitlecolo(255,default_transparent);
pom->settitle(lang("POZIOM SPECJALIZACJI AGENTÃW","LEVEL OF SPECIALISATION"));
MyAreaMenager.insert(pom);

method_matrix_source<agent,int>* SpecjalizacjaA=ziemia.make_source(lang("bity autotrofÃ³w","spec. bits of autotrophs"),&agent::how_specialised_autotrof);
...
method_matrix_source<agent,int>* SpecjalizacjaH=ziemia.make_source(lang("bity heterotrofÃ³w","spec. bits of heterotrophs"),&agent::how_specialised_heterotrof);
...
//AktualneSwiatlo????
template_scalar_source_base<double>* currLight=new ptr_to_scalar_source<double>(&AktualneSwiatlo,lang("NasÂ³onecznienie","Insolation"));
...
//Bogactwo klonÃ³w i gatunkÃ³w
template_scalar_source_base<unsigned long>* allclons=
					new ptr_to_fuction_source<unsigned long>(informacja_klonalna::ile_klonow,lang("klonÃ³w od poczatku","clones from the beginning"));
...
template_scalar_source_base<unsigned long>* alltax=
					new ptr_to_fuction_source<unsigned long>(informacja_klonalna::ile_taksonow,lang("eko-gatunkÃ³w od poczatku","species from the begining"));
...
template_scalar_source_base<unsigned>* sklon=new ptr_to_scalar_source<unsigned>(&agent::ile_tax,lang("klony","clones"));// ile w ogÃ³le istniejacych klonow
...
pom1->setframe(128);
pom1->settitle(lang("AGENCI - HISTORIA LICZEBNOÅCI","History of the number of agents"));
//pom1->setdatacolors()
...
if(!pom1) goto DO_OBSLUGI_BLEDOW;
pom1->setframe(128);
pom1->settitle(lang("HISTORIA LICZBY TAKSONÃW","History of the number of taxa"));
MyAreaMenager.insert(pom1);
...
rectangle_source_base* Background=zdatnosc.make_source(lang("przydatnoÅÃŠ terenu","suitability of areas"));
...
pom->setbackground(default_half_gray);
pom->settitlecolo(255,default_transparent);
pom->settitle(lang("Mapa zdatnych obszarÃ³w","Map of useful/unusable areas"));
MyAreaMenager.insert(pom);
...
pom->setbackground(default_half_gray);
pom->settitlecolo(255,default_transparent);
pom->settitle(lang("WIEK AGENTÃW","AGE OF AGENTS"));
MyAreaMenager.insert(pom);
...
if(!pom1) goto DO_OBSLUGI_BLEDOW;
pom1->setframe(128);
pom1->settitle(lang("HISTORIA ENERGII AGENTÃW","HISTORY OF AGENTS ENERGY"));
MyAreaMenager.insert(pom1);
...
pom->setbackground(default_half_gray);
pom->settitlecolo(255,default_transparent);
pom->settitle(lang("LOKALNY PRZEPÂ£YW ENERGII","LOCAL ENERGY FLOW"));
MyAreaMenager.insert(pom);
...
pom->settitle(lang("MAPA UÂ¯YWANYCH NISZ EKOLOGICZNYCH","USAGE OF ECOLOGICAL NICHES"));
MyAreaMenager.insert(pom);
...
pom1->settitle(lang("FILOGENETYCZNY WIEK AGENTÃW","PHYLOGENETIC AGE OF AGENTS"));
MyAreaMenager.insert(pom1);
...
//HISTOGRAM SPECJALIZACJI AUTOTROFÃ³W
////////////////////////////////////////////////////////////////
...
pom->setframe(232);
pom->settitle(lang("SPECJALIZACJA AUTOTROFÃW","SPECIALISATION OF AUTOTHROPS"));
MyAreaMenager.insert(pom);

//HISTOGRAM SPECJALIZACJI HETEROTROFÃW
/////////////////////////////////////////////////////////////////
...
pom->setframe(32);
pom->settitle(lang("SPECJALIZACJA HETEROTROFÃW","SPECIALISATION OF HETEROTROPHS"));
MyAreaMenager.insert(pom);

//FILOGENEZA TAXONÃW -  MOZE BYC KOSZTOWNE W RYSOWANIU
//////////////////////////////////////////////////////////////////////////
...
wb_pchar buf(1024);
buf.prn(lang("FILOGENEZA - taksony powyÂ¿ej %d agentÃ³w",
			 "PHYLOGENETIC TREE - taxons larger than %d individuals"),REJESTROWANY_ROZMIAR_TAKSONU-1);
...
{
	//Tu byÃŠ moÂ¿e trzeba cos dodaÃŠ, ale na razie tak - bez kontroli
	//...
	//Informacje nagÂ³owkowe
...
swiat::swiat(size_t szerokosc,char* logname,char* mappname,my_area_menager& AreaMenager):
	...
	TrophicNet(NULL),		//SieÃŠ zaleznoÅci pokarmowych
	Liczniki(NULL),
	licznik_krokow_w(0),
	monte_carlo_licz(0),
	Wiek(NULL),Sila(NULL),
...
lang("EKSPLOATACJA DRAPIEZNICZA=\t",
...
klon->dolicz_indywiduum();//Klonalny licznik indywiduÃ³w
ile_ind++;                //Globalny licznik indywiduÃ³w
...
if( liczniki[iwfull]==1 ) // pierwszy przedstawiciel taxonu
		ile_tax++;   // wiec liczba klonÃ³w wzrasta

if( liczniki[iwfull]==informacja_klonalna::tresh_taksonow()+1 ) // osiagnal w gÃ³re wartosc == duzy taxon - rozwojowy
		ile_big_tax++;
...
if( liczniki[wfull]==0 )	//ostatni przedstawiciel tego taxonu
	ile_tax--;
if( liczniki[wfull]==informacja_klonalna::tresh_taksonow()) // osiagnal w dÃ³l wartosc == tresh_kasowania --> maly taxon
	ile_big_tax--;
assert(w._full>0 );
...
if(WSP_KATASTROF!=0)
	kataklizm(); // najwyÅŒej jeden, chocby bardzo bardzo maly na krok symulacji
...
monte_carlo_licz+=MonteCarloMultiplic;//Licznik krokÃ³w MonteCarlo - bezwzglÃªdna jednostka czasu
...
for(long i=0;i<ile;i++)
	...
	int			x1,y1;
	do{ //Losowanie kierunku ruchu - ??? mozna by trochÃª przyÅpieszyÃŠ ale chyba tylko trochÃª...
	ZNOWU:
		...
		if((licznik++)==8) goto JESZCZE_RAZ;//Wyskok kontynuujacy petle zewnetrznÂ¹
	...
	//Koniec krokÃ³w monte-carlo po agentach
	//------------------------------------------  NIE W TYM MIEJSCU!!!
...
bufor[2047]='A';//MoÂ¿e zostaÃŠ albo A albo NULL ale jak coÅ innego to przepelniony

sprintf(bufor,lang("%ld KROK MONTE CARLO [%lu - symulacji]   PID=%lu\n"
							"LICZBA AGENTOW:%lu "
							"(%lu AUTOTROFICZNYCH)\n"
							"LICZBA EKO-GATUNKÃW:%lu "
							"(%lu AUTOTROFICZNYCH)\n"
							"LICZBA KLONOW:%lu "
							"(%lu AUTOTROFICZNYCH)\n"
							"ILE KLONÃW OD POCZATKU:%lu "
							"(%lu ISTOTNYCH)",
...
{
cout<<lang("CO-EWOLUCJA: program symulujacy kooewolucje wielu gatunkow (do 65000)\n",
...
my_area_menager Lufciki(255,SWIDTH,SHEIGHT,28);//255 moÂ¿liwych lufcikÃ³w
swiat& tenSwiat=*new swiat(IBOKSWIATA,LogName,MappName,Lufciki);
if(&tenSwiat==NULL)
{
	cerr<<lang("Brak pamieci!\n","Not enought memory!\n")<<endl;
	exit(1);
}
...
{
	cerr<<lang("Nie moge wystartowac grafiki","Can't initialize graphics")<<endl;
    exit(1);
}
...
printf(lang("\nPID procesu: %u Marker zrzutÃ³w ekranu: %lu","\nProcess PID: %u Dump files marker: %lu"),_getpid(),tenSwiat.dumpmarker.get());
}

//zamiast Lufciki.run_input_loop();
cerr<<endl;

while(tenSwiat.daj_kroki_monte_carlo()<MAX_ITERATIONS) //Do zaÂ³oÂ¿onego koÃ±ca
	{
	...
	double czas_kroku=steptimer;
	 ...
	 //CiÂ¹gÂ³e zrzuty plikÃ³w jesli jest postrzeba
	 ...
	 if(DumpNETContinously)	 //stan sieci troficznej w formacie NET
	 {
		...
		tenSwiat.ZapiszEkologieNET(netstateout);//Bez ograniczeÃ±, tzn. z domyÅlnymi
		netstateout.close();
	 }
	 if(DumpVNAContinously)	 //stan sieci troficznej w formacie VNA
	 {
		...
		ofstream netstateout(bufor.get());
		tenSwiat.ZapiszEkologieVNA(netstateout);//Bez ograniczeÃ±, tzn. z domyÅlnymi
		netstateout.close();
	 }

	//OSTATECZNIE POMIAR CZASU OBROTU PÃTLI I NOWA PÃTLA
...
//Ostateczne zrzuty plikÃ³w do analizy
//Pelna filogeneza
ofstream filogout("lastfilog.out");
tenSwiat.ZapiszFilogeneze(filogout);//Bez ograniczeÃ±, tzn. z domyÅlnymi
filogout.close();
...
static bool firs_change_viewfreq=true;	//Do ostrzeÂ¿enia przy pierwszej
								//zmianie czÃªstotliowosci wizualizacji
	switch(input_char){
	case ID_VIEWOPT_DUMPCO:     //StaÂ³e zrzucanie grafiki
		{
		...
		return 1;
	case ID_VIEWOPT_DMPCNET:		//StaÂ³e zrzucanie sieci troficznej
		{
	...
	case ID_VIEWOPT_MOREOFT:
		...
		return 1;
	case ID_VIEWOPT_DMPCVNA:      	//StaÂ³e zrzucanie sieci troficznej w formacie VNA
		{
	...
	case ID_VIEWOPT_TRNETMORE:
		{
		  ...
		  cerr<<endl<<lang("Granica zauwaÅŒania wÄzÅÃ³w sieci troficznej obniÅŒona do ",
	...
	case ID_VIEWOPT_TRNETLESS:
		{
		  if(REJESTROWANY_ROZMIAR_WEZLA<1)
			...
			REJESTROWANY_ROZMIAR_WEZLA*=2;
		  cerr<<endl<<lang("Granica zauwaÅŒania wÃªzÂ³Ã³w sieci troficznej podwyÅŒszona do ",
						   "Important nodes of food web are now from ")<<REJESTROWANY_ROZMIAR_WEZLA<<endl;
		  MojSwiat->UstalDetalicznoscSieci(REJESTROWANY_ROZMIAR_WEZLA);
		}
		return 1;
//	#define ID_VIEWOPT_TRNETFULL            60012
	   case ID_VIEWOPT_TRNETFULL:
		{
		  REJESTROWANY_ROZMIAR_WEZLA=0;
		  cerr<<endl<<lang("Zniesiona granica zauwaÅŒania wÃªzÂ³Ã³w sieci troficznej",
...
//	#define ID_VIEWOPT_TRNETDEFA			60013
		case ID_VIEWOPT_TRNETDEFA:
		{
		  ...
		  cerr<<endl<<lang( "DomyÅlna granica zauwaÅŒania wÃªzÂ³Ã³w sieci troficznej (taka jak w drzewie fil.=",
		...
		case  ID_VIEWOPT_DMPNET:
		{
			...
			ofstream netstateout(bufor.get());
			MojSwiat->ZapiszEkologieNET(netstateout);//Bez ograniczeÃ±, tzn. z domyÅlnymi
			netstateout.close();
			bufor.prn("%s_%07dMC.vna",this->get_dump_name(),MojSwiat->daj_kroki_monte_carlo());//printf
			netstateout.open(bufor.get());
			MojSwiat->ZapiszEkologieVNA(netstateout);//Bez ograniczeÃ±, tzn. z domyÅlnymi
			netstateout.close();
		}
		return 1;
...
for(int i=1;i<argc;i++)
	...
	if(SWIDTH<50)
		{
		cerr<<lang("Za maÅe WIDTHWIN = ","Bad WIDTHWIN = ")<<SWIDTH<<lang(" (musi byÄ"," (must be")<<" >50)"<<endl;
		return 0;
		}
	...
	if(SHEIGHT<50)
		{
		cerr<<lang("Za maÅe HEIGHTWIN = ","Bad HEIGHTWIN = ")<<SHEIGHT<<lang(" (musi byÄ"," (must be")<<" >50)"<<endl;
		return 0;
		}
	...
	if(IBOKSWIATA<20 || IBOKSWIATA>=SWIDTH)
		{
		cerr<<"SIDE="<<IBOKSWIATA<<lang(" !!! Jest za maÂ³y lub wiÃªkszy niÅŒ deklarowany ekran!\n",
				   "!!! It is too small or wider than screen")<<IBOKSWIATA;
		cerr<<lang("WciÅnij ENTER jeÅli to akceptujesz lub przerwij program!",
				   "Press ENTER if it is acceptable or break the program");
		getchar();
		}
		else
		cerr<<lang("DÂ³ugi bok (SIDE) Åwiata ustawiony na: ",
				   "The longer side of simulation world SIDE=")<<IBOKSWIATA<<endl;
	}
	else
	//"FLEX=y/n - czy moÂ¿na zamieniaÃŠ siÃª z wÂ³aÅcicielem komorki\n"
	...
	if (ZAMIANY>255)
		{
		cerr<<"FLEX="<<ZAMIANY<<lang("!!! NiewÂ³aÅciwa wartoÅÃŠ. Powinno byÃŠ < 256",
	...
	//"RBIT=2^n - ktÃ³ry bit odpowiada za zdolnoÅÃŠ ruchu\n"
	...
	if (bit>1)
		{
		cerr<<lang("Bit ruchu (RBIT) musi byÃŠ ktÃ³rÂ¹Å potÃªgÂ¹ liczby 2. Np. ",
				   "The move bit (RBIT) must by a power of two. E.g. ")
				   <<" 0,1,2,4,8,16,32,128..."<<endl;
		cerr<<lang("A podana wartoÅÃŠ ma ",
				   "But given value have ")<<bit<<lang("bitÃ³w","bits")<<endl
			<<lang("JeÅli chcesz wyÂ³Â¹czyÃŠ podaj bit spoza maski obrony",
				   "To turn off provide power larger than defence mask capability.")<<endl;
		return 0;
		}
	else
		{
		cerr<<lang("Podano wartoÅÃŠ RBIT: ","Given value of RBIT is ")<<BIT_RUCHU<<endl;
		if(MAXBASE>BIT_RUCHU)
			...
			cerr<<lang("Bit ruchu jest poza maskÂ¹ obrony - wszyscy agenci majÄ moÅŒliwoÅÄ ruchu",
					   "The move bit is outside the defence mask - all agents are able to move")<<endl;
		}
	}
	else
	//MAXA=nnn - maksymalny wiek. Nie wiÃªkszy niÂ¿ 255. Ujemne oznacza Åmierci losowe\n"
	...
	if(MAX_WIEK>255)
		...
		cerr<<lang("!!! Maksymalny wiek (MAXA=",
			...
			<<lang(") nie moÅŒe byÃŠ wiÃªkszy niÅŒ 255!",
	...
	//"IFER=NN - prawdopodobieÃ±stwo rozmnaÂ¿ania to 1/IFER\n"
	...
	if(NIEPLODNOSC==0 || NIEPLODNOSC>1000)
		{
			cerr<<"!!! IFER = "<<NIEPLODNOSC
				<<lang("To musi byÃŠ liczba caÂ³kowita w zakresie 1..1000!",
		...
		{
		cerr<<"!!! IFER = "<<NIEPLODNOSC
			<<lang("czyli prawdopodobieÃ±stwo rozmnaÅŒania = ",
	...
	if((pom=strstr(argv[i],"PRED="))!=NULL) //Nie NULL czyli jest
	{
		...
		if(PIRACTWO)
		  ...
		  cerr<<lang("- eksploatacja pasoÅŒytnicza","- parasitic exploitation")<<endl;
	}
	else
	//"POSA=0.XX - jaki uÂ³amek sily oddane potomkowi\n"
	...
	if((pom=strstr(argv[i],"DOWR="))!=NULL) //Nie NULL czyli jest
	{
	  ...
	  cerr<<"DOWR="<<WYPOSAZENIE_POTOMSTWA<<lang(" zapasÃ³w rodzica.","of parent reserve.")<<endl;
	  if(WYPOSAZENIE_POTOMSTWA<=0 || 1<WYPOSAZENIE_POTOMSTWA)
	  {
		 cerr<<lang("!!! LICZBA MUSI BYÃ W ZAKRESIE (0..1) !!!",
	...
	if((pom=strstr(argv[i],"MUTD="))!=NULL) //Nie NULL czyli jest
	{
		...
		{
			cerr<<lang("MUTD musi byÄ wiÄksze lub rÃ³wne 16",
	...
	if((pom=strstr(argv[i],"CATA="))!=NULL) //Nie NULL czyli jest
	{
		...
		cerr<<lang("WspÃ³Åczynnik katastrof CATA=",
			   "Coefficient of disasters CATA=")<<WSP_KATASTROF<<endl;
		if(WSP_KATASTROF<=0)
			cerr<<lang(" - zatem katastrofy wyÅÄczone"," - so disasters are off")<<endl;
		else
			cerr<<lang(" - zatem katastrofy moÅŒliwe"," - so disasters are possible")<<endl;
	}
	else
	if((pom=strstr(argv[i],"AUTO="))!=NULL) //Nie NULL czyli jest
	{
		EFEKTYWNOSC_AUTOTROFA=atof(pom+5);
		cerr<<lang("ProduktywnoÅÄ autotrofÃ³w AUTO=",
				   "Productivity of autotrophs")<<EFEKTYWNOSC_AUTOTROFA;
		if(EFEKTYWNOSC_AUTOTROFA<=0)
		{
			cerr<<lang("Nie moÅŒe byc ujemna!!!",
		...
		{
		  cerr<<lang("Efektywnosc autotrofa powinna byÃŠ w zakresie 0..0.99 ale sprÃ³buje",
	...
	if((pom=strstr(argv[i],"DIST="))!=NULL) //Nie NULL czyli jest
	{
		...
		if(DYS_RUCHU<=0)
		{
			cerr<<lang("Nie moze byÄ <= 0","It can not be <= 0")<<endl;
			return 0;
		}
	}
	else
	...
	//"DMPF=name - rdzen/poczÂ¹tek nazwy plikÃ³w dump\n"
	//char     SCREENDUMPNAME[256]="CO-EVO-3a";
	if((pom=strstr(argv[i],"DMPF="))!=NULL) //Nie NULL czyli jest
	{
		strcpy(SCREENDUMPNAME,pom+5);
		cerr<<lang("RdzeÃ± (DMPF) nazwy zrzutÃ³w graficznych: ",
	...
	if((pom=strstr(argv[i],"TAXS="))!=NULL) //Nie NULL czyli jest
	{
		...
		cerr<<lang("Rozmiar rejestrowanych taksonÃ³w (TAXS) ustawiono na ",
				   "Minimal size of registered taxon (TAXS) ")<<REJESTROWANY_ROZMIAR_TAKSONU<<endl;
		if(REJESTROWANY_ROZMIAR_TAKSONU<10)
		{
			cerr<<lang("!!! Ale musi byÃŠ >=10 !!!","!!! But have to be >=10 !!!")<<endl;
			return 0;
		}
	}
	...
	cerr<<"===================================================================="<<endl;
	cerr<<lang("MOZLIWE TO: (W nawiasach wartoÅci domyÅlne)",
				"POSSIBLE ONES ARE: (default values in parentheses)")<<endl;
	cerr<<lang("SIDE=NN - dÅuÅŒszy bok obszaru symulacji ",
			   "SIDE=NN - the longer side of simulation world")<<'('<<IBOKSWIATA<<')'<<endl;//unsigned	IBOKSWIATA=300;// FAKTYCZNIE UZYWANY BOK SWIATA
	cerr<<lang("MAPF=plik_init.gif - nazwa pliku z mapa zamieszkiwalnoÅci",
			   "MAPF=init_file.gif - name of availability map file")<<'('<<MappName<<')'<<endl;//Inicjalizacja Åwiata - czarne, miejsca niezamieszkiwalne
	cerr<<lang("AUTO=0.XX - efektywnoÅÄ autotrofÃ³w ",
	...
	cerr<<lang("FLEX=y/n - czy moÅŒna zamieniaÃŠ siÃª z wÂ³aÅcicielem komorki ",
			   "FLEX=y/n - exchange position when agents bump")<<'('<<ZAMIANY<<')'<<endl;//const unsigned	ZAMIANY=255;//Czy moze przeciskac sie na zajete pola (0 na nie lub inna liczba na tak)
	cerr<<lang("RBIT=2^n - ktÃ³ry bit odpowiada za zdolnoÅÃŠ ruchu ",
			   "RBIT=2^n - weight of bit responsible for moving ability ")<<'('<<BIT_RUCHU<<')'<<endl;//const unsigned	BIT_RUCHU=1024;		// Poza maske - bez mozliwosci utraty ruchliwosci
	cerr<<lang("MUTD=NNN - co ile kopiowanych bitÃ³w trafia sie mutacja ",
			   "MUTD=NNN - how offen mutation occurs when copying bits ")<<"(every "<<PROMIENIOWANIE<<" bits)"<<endl;//unsigned	PROMIENIOWANIE=BITS_PER_GENOM*100;// Co ile kopiowanych bitow nastepuje mutacja
	cerr<<lang("DOWR=0.XX - jaki uÂ³amek energii oddane potomkowi ",
			   "DOWR=0.XX - dowry for offspring as % of parent energy ")<<'('<<WYPOSAZENIE_POTOMSTWA<<')'<<endl;//const double	WYPOSAZENIE_POTOMSTWA=0.05; // jaka czesc sily oddac potomkowi 0.1 to ZA DUZO!!! {???}
	cerr<<lang("IFER=NN - prawdopodobieÃ±stwo rozmnaÅŒania to. 1/IFER = 1/",
			   "IFER=NN - how offen agent try to make child. 1/IFER = 1/")<<'('<<NIEPLODNOSC<<')'<<endl;//const unsigned	NIEPLODNOSC=5;		// Prawdopodobienstwo rozmnazania jest 1/NIEPLODNOSC
	cerr<<lang("MAXA=nnn - maksymalny wiek. Nie wiÃªkszy niÅŒ 255. ",
			   "MAXA=nnn - maximal age. Not grater than 255. ")<<'('<<255-MINIMALNY_WIEK<<')'<<endl;// Ujemne BÃDZIE oznaczaÃŠ Åmierci losowe"<<'('<<xxx<<')'<<endl;//const unsigned	MINIMALNY_WIEK=205;	// Rodzi sie z tym wiekiem. Do smierci ma 255-MINIMALNY_WIEK
	...
	cerr<<lang("TAXS=NN - minimalny rozmiar taksonu rejestrowanego w drzewie. Nie mniej niÅŒ 10! ",
			   "TAXS=NN - minimal taxon size to be registered in phylogenetic tree. No less than 10! ")<<'('<<REJESTROWANY_ROZMIAR_TAKSONU<<')'<<endl;//unsigned	 REJESTROWANY_ROZMIAR_TAKSONU=MINIMALNY_ROZMIAR_TAKSONU;//Domyslna wartosc kasowania w wizualizacji drzewa
					//unsigned int	ODWROCONY_KOSZT_OSLONY=0;//1 - czy koszty oslony sa odwrocone
					//unsigned int	ODWROCONY_KOSZT_ATAKU=0;//1 - czy koszty ataku sa odwrocone

					//"MITA=NNN - minimalny rozmiar taksonu z def.\n"//unsigned	 MINIMALNY_ROZMIAR_TAKSONU=100;  //Arbitralny rozmiar klonu zeby go uznac za zarejestrowany takson
	cerr<<lang("INLO=NNNN - dlugoÅÃŠ wewnÃªtrznych logÃ³w ",
	...
	cerr<<lang("DMPF=nazwa - rdzen/poczÂ¹tek nazwy plikÃ³w dump ",
